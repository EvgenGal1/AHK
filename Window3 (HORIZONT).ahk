...
if (MonitorCount > 1) {
    ...
    ; проверка приложения
    if WinExist("ahk_exe " WindowChromeExe) {
        ; ожидает приложения
        WinWait("ahk_exe " WindowChromeExe)
        ; актвирует окно приложения
        WinActivate("ahk_exe " WindowChromeExe)
        ; откр.в раб.область на 1-ом мониторе
        WinMove(-1920, 0, A_ScreenWidth, A_ScreenHeight - 40, "ahk_exe " WindowChromeExe)
    }
    Sleep 250
}
...
if (MonitorCount > 1) {
    ...
    if WinExist("ahk_exe " WindowVSCExe) {
        ...
        ; откр.в раб.область на 2-ом мониторе на 80% с ЛЕВО
